...
public:
	...
	void SetTime(double seconds) { time_to_consume = seconds; }
	void AddTime(double seconds) { time_to_consume += seconds; }
	void Start() { last_chrono_time = steady_clock::now(); }
	void Pause() { paused = true; }
	bool IsExpired() const { return time_to_consume <= 0; }
	double GetRemainingTime() const { return time_to_consume; }
	double GetTotalTimeElapsed() const { return totalTimeElapsed; }
	void ResetTotalTime() { totalTimeElapsed = 0.0; }
